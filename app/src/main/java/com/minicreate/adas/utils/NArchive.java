...
public final class NArchive {
    ...
    private static final String TAG = "NArchive";
    ...
    private NArchive(boolean isload, byte[] buf, NFile f, String name, boolean useBuf) {
        LogUtil.d(TAG, "isload = " + isload + " ,buf == null ? " + (buf == null) + " ,f = " + f);
        m_isLoading = isload;
        if (useBuf) {
            if (buf == null) {
    ...
    public byte readalign1Bit(int len) {
        byte[] b = new byte[len];
        m_file.read(b, len);
        return b[0];
    }
    ...
    /**
     * 写入buffer，总长度为length，如果buffer的长度少于length，那么在后面补0
     *
     * @param buffer
     * @param length
     */
    ...
    public int readBytes(byte[] buffer) {
        return m_file.read(buffer, buffer.length);
    }
    ...
    public String readCharArray(int len) {
        ...
        m_file.read(b, len);
        int index = 0;
//		int index = Arrays.binarySearch(b, ((byte)0x00));
...
//		LogUtil.v(TAG, "index: " + index);
        ...
        if (index > 0 && index != (b.length)) {
            validData = Arrays.copyOfRange(b, 0, index);
        } else if (index == 0) {
            return "";
        } else {
            validData = b;
        ...
        try {
            String s = new String(validData, "gb2312");//"UTF-8");
            return s;
        } catch (Exception e) {
            e.printStackTrace();
        }
